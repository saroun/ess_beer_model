...
# %% USER INPUT - define primary parameters
...
def printCFG(dist, segments, gap=0.2):
    """Print config text for SIMRES segmented guide.
    """
...
def plotProfile(dist, coord='ISCS', file=''):
    ...
    NOTE: the distances are always in the default coordinates (_COORD)
    """
    ...
    if (len(file)>0):
        plt.savefig(file+'_hor'+'.png', bbox_inches='tight')
    plt.show()
    ...
    plt.grid()
    if (len(file)>0):
        plt.savefig(file+'_ver'+'.png', bbox_inches='tight')
    plt.show()
...
# in FP = FOC
plotProfile(d, coord='FP', file='BEER_profile_FP.png')
# in ISCS
plotProfile(d, coord='ISCS', file='BEER_profile_ISCS.png')

"""
    
#%%
"""
win = getSlice(2026, coord='ISCS')
printSlice(win, leg='at 2026')

win = getSlice(nboa_start, coord='ISCS')
printSlice(win, leg='at NBOA start')

win = getSlice(nboa_end, coord='ISCS')
printSlice(win, leg='at NBOA end')

win = getSlice(5403, coord='ISCS')
printSlice(win, leg='at 5403')

win = getSlice(bbg_start, coord='ISCS')
printSlice(win, leg='at BBG start')

win = getSlice(bbg_end, coord='ISCS')
printSlice(win, leg='at BBG end')

win = getSlice(5903, coord='ISCS')
printSlice(win, leg='at 5903')


win = getSlice(bbg_end, coord='FP')
printSlice(win, leg='at BBG end')
"""
...
# define segments
div = []
div.append([1, 1000., 2.5, 2.5, 4., 4.])
div.append([2, 1000., 2.5, 2.5, 4., 4.])
div.append([1, nboa_end-nboa_start-3000, 2.5, 2.5, 4., 4.])
div.append([1, 22.9, 0., 0., 0., 0.])
div.append([1, 481., 2.5, 2.5, 4., 4.])

out = printCFG(nboa_start, div)
print(out)
